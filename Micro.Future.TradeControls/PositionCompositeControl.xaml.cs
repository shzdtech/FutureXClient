...
namespace Micro.Future.UI
{
    ...
    /// </summary>
    public partial class PositionCompositeControl : UserControl, IReloadData, ILayoutAnchorableControl
    {
        ...
        private MarketDataHandler _ctpoptionMDHandler = MessageHandlerContainer.DefaultInstance.Get<MarketDataHandler>();
        private CTPETFMDHandler _ctpetfMDHandler = MessageHandlerContainer.DefaultInstance.Get<CTPETFMDHandler>();
        private CTPSTOCKMDHandler _ctpstockMDHandler = MessageHandlerContainer.DefaultInstance.Get<CTPSTOCKMDHandler>();
        ...
        public PositionCompositeControl(string persisitentId, string filterId, BaseTraderHandler tradeHander)
        {
            InitializeComponent();
        ...
        public void ReloadData()
        {
            ...
            {
                var positionctrl = new PositionCompositeControl(PersistanceId, fs.Id, TradeHandler);
        ...
        private async void LoadMarketData(string contract)
        {
            ...
            if (_ctpoptionMDHandler.MessageWrapper != null && _ctpoptionMDHandler.MessageWrapper.HasSignIn)
            {
                var subbedMD = await _ctpoptionMDHandler.SubMarketDataAsync(contract);
                if(subbedMD!=null)
                _marketDataList.Add(subbedMD);
            }
            ...
            {
                var subbedStockMD = await _ctpstockMDHandler.SubMarketDataAsync(contract);
                if (subbedStockMD != null)
                    _marketDataList.Add(subbedStockMD);
            }
        }
        ...
        private void MenuItem_Click_Position(object sender, RoutedEventArgs e)
        {
            if (AnchorablePane != null)
            {
                ...
                var positionctrl = new PositionCompositeControl(PersistanceId, Guid.NewGuid().ToString(), TradeHandler);
